...
async def create_knowledge_base(
    *,
    ...
    try:
        ...
        progress_tracker.update_progress(task_id, 5, "Knowledge base record created, preparing files...")
...
async def process_knowledge_base_async(
    ...
    with Session(engine) as session:
        try:
            ...
            progress_tracker.update_progress(task_id, 10, f"Starting file processing for {len(file_data)} files...")
            ...
            for i, file_info in enumerate(file_data):
                try:
                    ...
                    mock_file = MockUploadFile(file_obj, file_info['filename'], file_info['content_type'])
                    loaded_documents = load_uploaded_file(mock_file)
                    all_documents.extend(loaded_documents)
                    print(f"✅ Processed {file_info['filename']}: {len(loaded_documents)} documents")
            ...
            if not all_documents:
                progress_tracker.fail_task(task_id, "No documents could be processed from the uploaded files")
                return
            
            progress_tracker.update_progress(task_id, 45, f"Splitting {len(all_documents)} documents...")
            ...
            progress_tracker.update_progress(task_id, 50, "Initializing embeddings model...")
            ...
            document_chunks = chunk_documents_for_embedding(splits, max_tokens_per_chunk=settings.KB_EMBEDDING_CHUNK_SIZE)
            ...
            for i, chunk in enumerate(document_chunks):
                ...
                progress_tracker.update_progress(
                    task_id, 
                    int(progress_pct), 
                    f"Creating embeddings: {i+1}/{len(document_chunks)} ({progress_pct:.1f}%)"
                )
            ...
            progress_tracker.update_progress(task_id, 92, "Compressing vector database...")
            ...
            progress_tracker.update_progress(task_id, 100, "Knowledge base created successfully!")
            
            print(f"✅ Successfully created knowledge base '{knowledge_base.title}' with {len(file_data)} files")
            if failed_files:
            ...
            try:
                ...
                if kb_to_delete:
                    ...
                    session.commit()
                    print(f"🗑️ Cleaned up placeholder knowledge base {knowledge_base_id}")
