...
from app.models import TwinCheckRequest, TwinCheckResponse, TwinCheckTopicList, LlmInteraction, TwinCheckRequest
...
async def compare_documents(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail="Only text files are supported at this time")
...
async def get_comparison_history(
    ...
    try:
        ...
        for comparison in comparisons:
            try:
                input_data = json.loads(comparison.input_data) if comparison.input_data else {}
                output_data = json.loads(comparison.output_data) if comparison.output_data else {}
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Error retrieving comparison history: {str(e)}")
...
async def get_comparison_detail(
    ...
    try:
        ...
        if comparison.user_id != current_user.id:
            raise HTTPException(status_code=403, detail="You don't have access to this comparison")
        
        if comparison.functionality != "twincheck":
            raise HTTPException(status_code=400, detail="This is not a TwinCheck comparison")
        ...
        try:
            ...
            result = {
                ...
                "feedback": {
                    ...
                    "feedbackDate": comparison.feedback_date.isoformat() if comparison.feedback_date else None
                }
            }
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Error retrieving comparison details: {str(e)}")

# Functions related to Comparisons (saved comparison topics)
@router.post("/comparisons", response_model=TwinCheckTopicList)
def create_comparison(comparison: TwinCheckTopicList, session: SessionDep, current_user: CurrentUser):
    """
    ...
    if existing_comparison:
        raise HTTPException(status_code=400, detail="A comparison with this name already exists.")
...
def update_comparison(comparison_id: uuid.UUID, updated_comparison: TwinCheckTopicList, session: SessionDep, current_user: CurrentUser):
    """
...
def delete_comparison(comparison_id: uuid.UUID, session: SessionDep, current_user: CurrentUser):
    """
