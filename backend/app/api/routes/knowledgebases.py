...
def create_streaming_zip_from_directory(source_dir: str, temp_file_path: str):
    ...
    _, size_mb = MemoryManager.create_streaming_zip_from_directory(
        source_dir, temp_file_path
    )
    return size_mb
...
def read_file_with_memory_management(file_path: str) -> bytes:
    ...
    print(
        f"File size: {file_size_mb:.1f}MB, Available memory: {available_memory_mb:.1f}MB, Current usage: {current_usage_percent:.1f}%"
    )
    ...
    should_use_chunked = (
        file_size_mb > settings.KB_MAX_IN_MEMORY_SIZE_MB
        or file_size_mb > available_memory_mb * settings.KB_MEMORY_SAFETY_THRESHOLD
        or current_usage_percent > settings.KB_HIGH_MEMORY_USAGE_THRESHOLD
    )
    ...
    if should_use_chunked:
        ...
        if file_size_mb > settings.KB_MAX_IN_MEMORY_SIZE_MB:
            reason.append(
                f"exceeds size threshold ({settings.KB_MAX_IN_MEMORY_SIZE_MB}MB)"
            )
        if file_size_mb > available_memory_mb * settings.KB_MEMORY_SAFETY_THRESHOLD:
            reason.append(
                f"exceeds {settings.KB_MEMORY_SAFETY_THRESHOLD*100:.0f}% of available memory"
            )
        ...
        print(
            f"Small file ({file_size_mb:.1f}MB) with sufficient memory - loading normally"
        )
...
def read_file_in_chunks(file_path: str, chunk_size_mb: int = 8) -> bytes:
    ...
    print(
        f"Reading file in memory-safe mode: {file_size_mb:.1f}MB file, {available_memory_mb:.1f}MB available"
    )

    # If file is very large relative to available memory, use direct reading
    if (
        file_size_mb > available_memory_mb * 0.4
    ):  # File is more than 40% of available memory
        print(
            f"Large file detected - using direct file reading to avoid memory doubling"
        )
        try:
            with open(file_path, "rb") as f:
                file_data = f.read()
            ...
            print(
                "Direct reading failed due to memory constraints, using minimal chunking..."
            )
...
def _read_file_with_minimal_memory_footprint(file_path: str) -> bytes:
    ...
    try:
        ...
        print(
            f"Successfully read {total_bytes // 1024 // 1024}MB file using minimal memory approach"
        )
        return result
...
def log_progress(
    ...
    if task_id:
        ...
        # Calculate overall progress: (completed_steps + current_step_progress) / total_steps
...
def create_source_entry_from_file_data(
    session: SessionDep,
...
def read_knowledge_bases(
    ...
    logger.info(
        f"Returning {len(knowledge_bases)} knowledge bases for user {current_user.id}"
    )
    ...
    *,
    ...
    if embedding_model_id:
        try:
            ...
            raise HTTPException(
                status_code=422, detail="Invalid embedding_model_id format"
            )
    ...
    if settings.STREAMING_UPLOAD_ENABLED:
        ...
        for i, file in enumerate(files):
            ...
            if not is_valid:
                progress_tracker.fail_task(
                    task_id, f"File '{file.filename}' rejected: {reason}"
                )
                raise HTTPException(
                    status_code=400, detail=f"File '{file.filename}' rejected: {reason}"
                )

            # Stream file to temporary storage
            safe_filename = sanitize_filename(file.filename)
            temp_path = await StreamingFileHandler.stream_to_temp_storage(
                upload_file=file, temp_dir=temp_dir, filename=safe_filename
            )

            # Validate the streamed file
            is_valid, reason = FileValidator.validate_temp_file(
            ...
            if not is_valid:
                ...
                if os.path.exists(temp_path):
                    os.remove(temp_path)
                progress_tracker.fail_task(
                    task_id, f"File '{safe_filename}' validation failed: {reason}"
                )
                raise HTTPException(
                    status_code=400,
                    detail=f"File '{safe_filename}' validation failed: {reason}",
                )
    ...
    print(
        f"📊 Processed {len(files)} files totaling {total_size / (1024*1024):.1f}MB (validated and sanitized)"
    )
    ...
    print(
        f"🎯 SCHEDULING BACKGROUND TASK for KB {knowledge_base.id} with task {task_id}"
    )
...
async def process_knowledge_base_background(
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            for i, file_info in enumerate(file_data):
                ...
                with open(temp_file_path, "wb") as temp_file:
                    ...
                    print(
                        f"✅ Saved {file_info['filename']} ({file_info['size'] / (1024*1024):.1f}MB)"
                    )
        ...
        logger.error(
            f"Background processing error for KB {knowledge_base_id}: {str(e)}"
        )
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            if not os.path.exists(temp_dir):
                progress_tracker.fail_task(
                    task_id, f"Temp directory not found: {temp_dir}"
                )
                return
        ...
        # Mark task as failed
        progress_tracker.fail_task(
            task_id, f"Streaming background processing error: {str(e)}"
        )
        logger.error(
            f"Streaming background processing error for KB {knowledge_base_id}: {str(e)}"
        )
        ...
        try:
            ...
            with error_recovery_context("document processing"):
                ...
                for retry in range(max_retries):
                    ...
                    print(
                        f"🔍 CHECKING UPLOAD STATUS: progress exists={progress is not None}"
                    )
                    if progress:
                        upload_stage = progress.get("stages", {}).get("upload", {})
                        completed = upload_stage.get("completed", False)
                        print(
                            f"🔍 UPLOAD STAGE: completed={completed}, stage_data={upload_stage}"
                        )
                        if completed:
                    ...
                    if retry < max_retries - 1:  # Don't sleep on last iteration
                        print(
                            f"⏳ Waiting for upload completion... (retry {retry + 1}/{max_retries})"
                        )
                ...
                if not upload_completed:
                    ...
                    print(
                        f"⚠️ Upload stage not completed by middleware, completing it now"
                    )
            ...
            with error_recovery_context("document splitting"):
                ...
                for i, doc in enumerate(all_documents):
                    ...
                    print(
                        f"📄 Chunked text {i + 1}/{total_documents}: {len(doc_chunks)} chunks"
                    )
            ...
            with error_recovery_context("embedding initialization"):
                ...
                embeddings, model_id, provider = load_correct_embeddings_model(
                    session=session,
                    current_user=None,  # Pass None since we only have user_id
                    embedding_model_id=embedding_model_id,
                )
                print(f"Using embedding model: {model_id}")
            ...
            with error_recovery_context("vector database creation"):
                ...
                try:
                    ...
                    for i, chunk in enumerate(document_chunks):
                        ...
                        # Ensure chunk is not empty before processing
                        valid_documents = [
                            doc for doc in chunk if doc.page_content.strip()
                        ]
                        if not valid_documents:
                            print(
                                f"Chunk {i+1} contains no valid documents, skipping..."
                            )
                            continue
                        ...
                        if chroma_db is None:
                            ...
                            print(
                                f"🧠 Creating initial vector database with {len(valid_documents)} documents..."
                            )
                            ...
                            try:
                                ...
                                print(
                                    f"✅ Initial vector database created with {len(valid_documents)} documents"
                                )
                            ...
                            print(
                                f"🧠 Adding {len(valid_documents)} documents to existing vector database..."
                            )
                            ...
                            try:
                                ...
                                print(
                                    f"✅ Added {len(valid_documents)} documents to vector database"
                                )
                        ...
                        if memory_percent > 80:  # Critical memory usage
                            print(
                                f"Critical memory usage: {memory_percent:.1f}%, forcing cleanup"
                            )
                            gc.collect()
                            await asyncio.sleep(2.0)
                        elif memory_increase > 200:  # Large memory increase
                            print(
                                f"Large memory increase detected: +{memory_increase:.1f}MB"
                            )
                            gc.collect()
                        ...
                        if memory_percent > 90:
                            print(
                                "CRITICAL: Memory usage above 90%, stopping processing"
                            )
                            raise HTTPException(
                                status_code=507,
            ...
            try:
                ...
                with error_recovery_context("knowledge base creation"):
                    ...
                    use_file_storage = (
                        zip_size_mb > settings.KB_USE_FILE_STORAGE_ABOVE_MB
                    )

                    if use_file_storage:
                        print(
                            f"Large file ({zip_size_mb:.1f}MB) - using file-based storage instead of database"
                        )
                        ...
                        print(
                            f"Small file ({zip_size_mb:.1f}MB) - using database storage"
                        )

                        # Check memory availability before reading file for database storage
                        available_memory = MemoryManager.get_memory_info()[
                            "system_available_mb"
                        ]
                        required_memory = (
                            zip_size_mb * 2
                        )  # Estimate 2x file size needed for safe operation

                        print(
                            f"Memory check: {zip_size_mb:.1f}MB file, {available_memory:.1f}MB available, {required_memory:.1f}MB required"
                        )

                        if (
                            required_memory > available_memory * 0.7
                        ):  # Need more than 70% of available memory
                            print(
                                f"WARNING: Large file may cause memory issues. Using optimized reading strategy."
                            )
                        ...
                        print(
                            f"Reading ZIP file with memory management: {zip_size_mb:.1f}MB"
                        )
                        knowledge_base.data = read_file_with_memory_management(
                            temp_zip_path
                        )
                ...
                if not (
                    hasattr(knowledge_base, "storage_type")
                    and knowledge_base.storage_type == "file"
                ):
            ...
            logger.info(
                f"📊 FINAL PROGRESS: Completing finalizing stage for task {task_id}"
            )
            ...
            logger.info(
                f"📊 FINAL PROGRESS: Stage completion result: {completion_success}"
            )
            logger.info(
                f"📊 FINAL PROGRESS: Task {task_id} should now be 100% complete"
            )

        except HTTPException as http_ex:
            # Mark task as failed for HTTP exceptions
            progress_tracker.fail_task(
                task_id, f"Knowledge base creation failed: {str(http_ex.detail)}"
            )
    ...
    if files or knowledge_base_in.removed_file_ids:
        ...
        try:
            ...
            if knowledge_base.storage_type == "file" and knowledge_base.file_path:
                ...
                print(
                    f"Reading knowledge base from file storage: {knowledge_base.file_path}"
                )
                if not os.path.exists(knowledge_base.file_path):
                    raise HTTPException(
                        status_code=500,
                        detail="Knowledge base file not found in storage",
                    )
            ...
            try:
                ...
                if use_file_storage:
                    print(
                        f"Large update ({zip_size_mb:.1f}MB) - using file-based storage"
                    )

                    # Clean up old file storage if it exists
                    if (
                        knowledge_base.storage_type == "file"
                        and knowledge_base.file_path
                    ):
                    ...
                    print(
                        f"Small update ({zip_size_mb:.1f}MB) - using database storage"
                    )
                    ...
                    print(
                        f"Memory check for update: {zip_size_mb:.1f}MB file, {available_memory:.1f}MB available, {required_memory:.1f}MB required"
                    )

                    if required_memory > available_memory * 0.7:
                        print(
                            f"WARNING: Large file update may cause memory issues. Using optimized strategy."
                        )
                        gc.collect()
