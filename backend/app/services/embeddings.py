    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            if isinstance(output, list):
                ...
                if len(output) == len(texts) and all(isinstance(emb, list) for emb in output):
                    print("Batch output appears to be one embedding per text")
                    return output
                    
                # If triple-nested structure: [[[values for text 1]], [[values for text 2]], ...]
                elif len(output) == len(texts) and all(isinstance(x, list) and len(x) == 1 and isinstance(x[0], list) for x in output):
                ...
                elif len(output) == 1 and isinstance(output[0], list) and len(output[0]) == len(texts):
                ...
                # Handle unexpected structures by falling back to one-by-one embedding
                    ...
                    print(f"Complex batch structure, falling back to individual embedding")
                    return [self.embed_query(text) for text in texts]
            else:
                print(f"Unexpected batch output type: {type(output)}, falling back to individual embedding")
                return [self.embed_query(text) for text in texts]
                
        except Exception as batch_e:
            print(f"Batch embedding failed: {str(batch_e)}, falling back to individual embedding")
            import traceback
            traceback.print_exc()
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        try:
            ...
            if isinstance(output, list):
                ...
                elif len(output) == 1 and isinstance(output[0], list) and all(isinstance(x, (float, int)) for x in output[0]):
                    print("Output is a list containing a single list of numbers")
                    return output[0]
                    
                # Case 3: Triple-nested list [[[0.1, 0.2, ...]]]
                elif len(output) == 1 and isinstance(output[0], list) and len(output[0]) == 1 and isinstance(output[0][0], list):
                    print("Output is a triple-nested list - extracting inner values")
                    return output[0][0]
                    
                # Case 4: List of lists for multiple embeddings [[0.1, 0.2, ...], [0.3, 0.4, ...]]
                elif all(isinstance(x, list) and all(isinstance(y, (float, int)) for y in x) for x in output):
                    print("Output is a list of embedding lists - taking first embedding")
                    return output[0]
                
                # Try to find any list of floats in the structure
                else:
                    print(f"Complex output structure: {type(output)} with length {len(output)}")
                    
                    # If it's a list with one element that's also a list
                    if len(output) == 1 and isinstance(output[0], list):
                        inner = output[0]
                        print(f"Examining inner list of type {type(inner)} with length {len(inner)}")
                        
                        # Keep unwrapping lists until we find a list of floats
                        current = output
                        depth = 0
                        while depth < 5 and isinstance(current, list) and len(current) > 0:
                        ...
                        print(f"Couldn't find valid embedding after {depth} levels of unwrapping")
                    
                    # If it's just a strange structure, log details and raise error
                    print(f"Unexpected structure. First element type: {type(output[0]) if len(output) > 0 else 'N/A'}")
                    raise ValueError(f"Couldn't extract embeddings from complex output structure: {str(output)[:100]}...")
