...
class SessionManager:
    ...
    def __init__(self):
        self.default_ttl = 86400  # 24 hours - extended from 60 minutes to prevent session expiration during document analysis
        ...
        try:
            ...
            print(
                f"SessionManager: Failed to connect to Redis ({e}), using in-memory fallback"
            )
    ...
    def session_needs_rebuild(
        self, session_data: Dict[str, Any], component: str
    ) -> bool:
        """
        ...
        if isinstance(component_data, (str, int, float, bool, list)):
            print(
                f"Component {component} is basic type {type(component_data)} - needs rebuilding"
            )
            return True
        ...
        if component == "retriever":
            ...
            if not has_method:
                print(
                    f"Retriever missing get_relevant_documents method - needs rebuilding"
                )
            ...
            if not has_method:
                print(
                    f"Vectorstore missing similarity_search method - needs rebuilding"
                )
            return not has_method
