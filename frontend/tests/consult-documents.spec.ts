...
test.describe('Consult Documents Toggle', () => {
  ...
  test('should toggle consult documents for individual questions', async ({ page }) => {
    ...
    const knowledgeBaseSection = page.locator('[data-testid="knowledge-base-section"]')
    ...
    if (await checklistSection.isVisible()) {
      ...
      if (await createChecklistButton.isVisible()) {
        ...
        await page.fill('input[placeholder*="checklist name"]', 'Test Checklist')
        await page.fill('textarea[placeholder*="description"]', 'Test checklist for consult documents toggle')
        
        // Add a few questions
        await page.fill('textarea[placeholder*="Enter question"]', 'Question 1: Test question with consult documents enabled')
        await page.keyboard.press('Tab')
        
        // Add second question
        const secondQuestionInput = page.locator('textarea[placeholder*="Enter question"]').nth(1)
        await secondQuestionInput.fill('Question 2: Test question with consult documents disabled')
        
        // Toggle off consult documents for the second question
        const consultDocumentsToggles = page.locator('input[type="checkbox"][aria-label*="Consult documents"]')
        ...
        const testChecklistRow = page.locator('text=Test Checklist').locator('..')
        await testChecklistRow.locator('input[type="checkbox"]').check()
        
        console.log('✅ Consult documents toggle test setup completed')
        console.log('✅ Created checklist with mixed consult documents settings')
  ...
  test('should maintain toggle states when editing checklist', async ({ page }) => {
    ...
    if (await testChecklist.isVisible()) {
      ...
      const editButton = checklistRow.locator('button[aria-label="View checklist"]')
      ...
      test.skip(true, 'Test checklist not found - run the first test to create it')
    }
  })
})
