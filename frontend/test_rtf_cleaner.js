...
const testCases = [
  ...
  },
  {
    ...
    input: "{\\rtf1\\ansi\\deff0 {\\fonttbl {\\f0 Times New Roman;}}\\f0\\fs24 This is some text with \\pard\\tx220\\tx720\\fi-360\\bullet Here is a bullet point\\par Another line with \\\\ backslashes\\par Text with \\~non-breaking~space and \\'e9accented\\'e0characters.}",
    expected: "This is some text with • Here is a bullet point\nAnother line with backslashes\nText with non-breakingspace and éaccentedàcharacters."
  },
  {
...
testCases.forEach((testCase, index) => {
  ...
  try {
    ...
    if (!passed) {
      console.log(`Difference: Expected length ${testCase.expected.length}, got ${result.length}`);
    }
  } catch (error) {
