...
const SectionEditor: React.FC<SectionEditorProps> = ({
  ...
  useEffect(() => {
    ...
    console.log("SectionEditor: lastSectionsValueRef.current", lastSectionsValueRef.current)
    
    // Always initialize if we have no items, regardless of sections prop
    if (sectionItems.length === 0 || sections !== lastSectionsValueRef.current) {
      ...
      // Only run this if we didn't successfully parse above OR if we have no items
