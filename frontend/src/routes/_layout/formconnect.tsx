  ...
  const handleProcessBatch = async () => {
    ...
    try {
      ...
      for (let fileIndex = 0; fileIndex < fileCount; fileIndex++) {
        ...
        const digitizedFiles = rowFiles
          .filter((item) => !item.isHandwritten)
          .map((item) => item.file)
        const handwrittenFiles = rowFiles
          .filter((item) => item.isHandwritten)
          .map((item) => item.file)
            ...
            <Box overflowX="auto" width="100%">
              ...
              <Table.Root variant="simple" width="100%">
                ...
                <Table.Body>
                  ...
                  {Array.from({
                    length: Math.max(1, ...batchFileItems.map((item) => item.files.length)),
                  }).map((_, rowIndex) => (
            ...
            <Box>
              ...
              >
                {batchLoading ? (
                  ...
                  <Text color="gray.500">
                    Results will appear here after processing batch files.
                  </Text>
                )}
              </Box>
            </Box>
      ...
      <HStack>
        <ChakraField.Root display="flex" alignItems="center" width="auto">
          ...
          <Switch.Root id={`batch-handwritten-${index}`} colorPalette="blue" size="sm">
            <Switch.HiddenInput checked={isHandwritten} onChange={onToggleHandwritten} />
            <Switch.Control>
              <Switch.Thumb />
            </Switch.Control>
          </Switch.Root>
