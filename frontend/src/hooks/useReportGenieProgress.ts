...
export const useReportGenieProgress = (taskId: string | null) => {
    console.warn("🔄 REPORTGENIE PROGRESS HOOK CALLED:", new Date().toISOString(), "taskId:", taskId)
    ...
    useEffect(() => {
        ...
        console.log(`🚀 REPORTGENIE HOOK EFFECT TRIGGERED at ${timestamp} with taskId:`, taskId)
        ...
        if (!taskId) {
            ...
            console.log("🔄 No taskId - resetting ReportGenie progress state to initial")
            return
        }

        // Force immediate reset of all state to prevent cached completion
        console.log("🔄 New ReportGenie task started, FORCE resetting ALL progress state for:", taskId)
        setProgress({
            percentage: 0,
            message: "Starting...",
            isActive: true,
        ...
        console.log(`🔄 Starting ReportGenie progress polling for task: ${taskId} at ${timestamp}`)
        ...
        const pollProgress = async () => {
            ...
            pollCount++
            const timestamp = new Date().toISOString()
            console.log(`📡 REPORTGENIE POLL #${pollCount} at ${timestamp} for task:`, taskId)

            try {
                console.log("🔐 Making authenticated request to ReportGenie progress API for task:", taskId)
                ...
                if ((data as any).status === "completed" || (data as any).status === "failed") {
                    console.log(`🏁 REPORTGENIE TASK ${(data as any).status.toUpperCase()}. Stopping polling at poll #${pollCount}`)
                    isPolling = false
        ...
        // Cleanup function
        return () => {
            console.log(`🧹 REPORTGENIE CLEANUP: Stopping polling for task ${taskId} after ${pollCount} polls`)
            isPolling = false
            clearInterval(intervalId)
        }
    }, [taskId])
