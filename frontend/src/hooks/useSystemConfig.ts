...
export const useSystemConfig = () => {
  return useQuery<SystemConfig>({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        console.error("🔍 useSystemConfig: API call failed with status:", response.status)
        throw new Error(`Failed to fetch system config: ${response.status}`)
      }
      const data = await response.json()
      console.log("🔍 useSystemConfig: API response:", data)
      console.log("🔍 useSystemConfig: enable_model_selection =", data.enable_model_selection)
      return data
    },
